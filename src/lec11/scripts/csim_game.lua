...
function csim_game.load()
	...
	local player_rigid_body = csim_rigidbody:new(1, 1, 14)
	player:addComponent(player_rigid_body)
	player.has_pressed_key = false
...
function csim_game.update(dt)
	if(player) then
		...
		csim_camera.y = player.pos.y - csim_game.game_height/2

		if(player.has_pressed_key) then
			csim_camera.x = csim_camera.x + camera_speed
			player.distance = player.distance + camera_speed

			if(player.distance > wave * map.tilewidth * 20) then
				csim_debug.text(wave)
				camera_speed = camera_speed * 1.2
				wave = wave + 1
			end

			csim_hud.distance = player.distance
		end

		csim_camera.x = csim_math.clamp(csim_camera.x, 0, map.tilewidth*300)

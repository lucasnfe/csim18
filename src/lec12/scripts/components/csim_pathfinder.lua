...
local csim_pathfinder = class()

function csim_pathfinder:init(map)
    self.map = map
...
function csim_pathfinder:adj(node)
    ...
    if(not up_tile.properties["collide"]) then
        ...
        local up_vector = csim_vector(node.x, node.y - 1)
    ...
    if(not down_tile.properties["collide"]) then
        ...
        local down_vector = csim_vector(node.x, node.y + 1)
    ...
    if(not left_tile.properties["collide"]) then
        ...
        local left_vector = csim_vector(node.x - 1, node.y)
    ...
    if(not right_tile.properties["collide"]) then
        ...
        local right_vector = csim_vector(node.x + 1, node.y)

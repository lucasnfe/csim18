...
function csim_collider:detectHorizontalCollision(vel)
    ...
    local col_pos1 = csim_vector(self:getColliderWorldPos().x + self.rect.w,
                                 self:getColliderWorldPos().y + SKIN)
    local col_pos2 = csim_vector(self:getColliderWorldPos().x + self.rect.w,
                                 self:getColliderWorldPos().y + self.rect.h - SKIN)

    if(horiz_side == -1) then
        col_pos1 = csim_vector(self:getColliderWorldPos().x,
                               self:getColliderWorldPos().y + SKIN)
        col_pos2 = csim_vector(self:getColliderWorldPos().x,
                               self:getColliderWorldPos().y + self.rect.h - SKIN)
    end
...
function csim_collider:detectVerticalCollision(vel)
    ...
    local col_pos1 = csim_vector(self:getColliderWorldPos().x + SKIN,
                                 self:getColliderWorldPos().y + self.rect.h)
    local col_pos2 = csim_vector(self:getColliderWorldPos().x + self.rect.w - SKIN,
                                 self:getColliderWorldPos().y + self.rect.h)

    if(vert_side == -1) then
        col_pos1 = csim_vector(self:getColliderWorldPos().x + SKIN,
                               self:getColliderWorldPos().y)
        col_pos2 = csim_vector(self:getColliderWorldPos().x + self.rect.w - SKIN,
                               self:getColliderWorldPos().y)
    end
...
function csim_collider:didCollideVertically(tile_x, tile_y, vert_side)
    ...
    if(vert_side == -1) then
        self.parent.pos.y = screen_y + self.parent.height - self.rect.y
...
function csim_collider:didCollideHorizontally(tile_x, tile_y, horiz_side)
    ...
    screen_x, screen_y = self.map:convertTileToPixel(tile_x, tile_y)
    if(horiz_side == -1) then
        self.parent.pos.x = screen_x + self.parent.width - self.rect.x
    elseif(horiz_side == 1) then
        self.parent.pos.x = screen_x - self.rect.x - self.rect.w
    end
end

function csim_collider:getAABB()
    local aabb = {}
    aabb.min = csim_vector(self.parent.pos.x + self.rect.x,
                           self.parent.pos.y + self.rect.y)

    aabb.max = csim_vector(aabb.min.x + self.rect.w,
                           aabb.min.y + self.rect.h)
    return aabb
end

...
function love.load()
	...
	if(player and player.load) then
		player:load()
	end
	...
	sound = love.audio.newSource("sounds/lec12.wav", "static")
...
function love.update(dt)
	if(player) then
		...
		camera.x = csim_math.clamp(camera.x, 0, 8*9)
		camera.y = csim_math.clamp(camera.y, 0, 8*9)
	end
